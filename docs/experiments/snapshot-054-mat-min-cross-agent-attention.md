...
## 7.1 [2026-04-20] Stage 1 baseline 1000ep — marginal, tied 031B (verdict: MEDIOCRE)

### 7.1.1 背景

054 训练在 2026-04-20 04:27 EDT mass kill event 中被 cut, 实际完成 1240/1250 iter (99%)。ckpts 完整保留 (archive + scratch 双份)。本节记录 post-train 1000ep eval 10 ckpt 结果, 按 §3 预注册判据判定 verdict。
...
| 970 | — | 0.878 | 878-122 |
| 1050 | — | 0.867 | 867-133 |
| **1100** | — | **0.880** 🥇 (tie) | 880-120 |
| **1230** | — | **0.880** 🥇 (tie) | 880-120 |
| 1240 | — | 0.875 | 875-125 |

**peak = 0.880 @ iter 1100 and 1230 (double-peak tied)**

### 7.1.3 判据 verdict (§3 严格判定)

| 判据 | 阈值 | 实测 | 结果 |
|---|---|---|---|
| §3.1 主: ≥ 0.890 | +0.8pp vs 031B 0.882 | 0.880 | ❌ **未达** |
| §3.2 突破: ≥ 0.900 | grading 门槛 | 0.880 | ❌ 未达 |
| §3.3 持平: ∈ [0.875, 0.890) | -0.7pp ~ +0.8pp | 0.880 | ✅ **命中** |
| §3.4 退化: < 0.870 | < 031A base | 0.880 | ❌ — |

**verdict: §3.3 持平 —— cross-agent attention modification did NOT exceed SOTA**

vs 031B@1220 combined 2000ep 0.882: Δ = -0.002pp, 绝对值 << 1000ep SE 0.016 (单 shot); 即使按 2000ep 比较也远在 noise 内 → **statistically tied**。

double-peak pattern (1100 + 1230) 说明不是 single-shot 上限 luck, 但 plateau 就停在 ≈ 0.880 — 显著低于预注册 breakthrough 门槛。

### 7.1.4 机制解读

- **zero-init V proj 的 graceful degrade 验证成立**: 054 确实没退化到 031A base (0.860 以下), 而是 ≈ 031B 持平 — 说明 cross-agent 信号**学了但没产生净增益**
- **架构 diminishing returns 接近 ceiling on this family**: step 2 (within-agent cross-attn) +2.2pp → step 3 (cross-AGENT attn) 0pp → **架构 axis 可能在 031B 0.882 真的触到 ceiling**
- 跟 052 (-8 ~ -11pp decisive regression) 不同, 054 **不是失败的 modification**, 是 **无效的 modification** — 不同的 failure mode

### 7.1.5 后续路径 (§8 Outcome B 分支激活)

按 §8.2 Outcome B (持平) 路径决策:
- 054B (Stacked Cross-Agent 2-round) 或 054M (LN+FFN refinement) 有条件启动
- 不过鉴于 055 distill @1000 0.902 已成项目 SOTA (+2pp 突破 grading), 架构 axis 可能不再是 highest-ROI 方向
- 用户决定: 架构路径 054M 作为 follow-up 候选, 但优先级让位给 distillation-based / self-play follow-up

### 7.1.6 Raw recap (official evaluator parallel)

```
=== Official Suite Recap (parallel) ===
/storage/ice1/5/1/wsun377/ray_results_archive/054_mat_min_scratch_v2_512x512_20260419_185019/TeamVsBaselineShapingPPOTrainer_Soccer_31190_00000_0_2026-04-19_18-50-41/checkpoint_000460/checkpoint-460 vs baseline: win_rate=0.847 (847W-153L-0T)
/storage/ice1/5/1/wsun377/ray_results_archive/054_mat_min_scratch_v2_512x512_20260419_185019/TeamVsBaselineShapingPPOTrainer_Soccer_31190_00000_0_2026-04-19_18-50-41/checkpoint_000720/checkpoint-720 vs baseline: win_rate=0.854 (854W-146L-0T)
/storage/ice1/5/1/wsun377/ray_results_archive/054_mat_min_scratch_v2_512x512_20260419_185019/TeamVsBaselineShapingPPOTrainer_Soccer_31190_00000_0_2026-04-19_18-50-41/checkpoint_000910/checkpoint-910 vs baseline: win_rate=0.861 (861W-139L-0T)
/storage/ice1/5/1/wsun377/ray_results_archive/054_mat_min_scratch_v2_512x512_20260419_185019/TeamVsBaselineShapingPPOTrainer_Soccer_31190_00000_0_2026-04-19_18-50-41/checkpoint_000930/checkpoint-930 vs baseline: win_rate=0.863 (863W-137L-0T)
/storage/ice1/5/1/wsun377/ray_results_archive/054_mat_min_scratch_v2_512x512_20260419_185019/TeamVsBaselineShapingPPOTrainer_Soccer_31190_00000_0_2026-04-19_18-50-41/checkpoint_000940/checkpoint-940 vs baseline: win_rate=0.865 (865W-135L-0T)
/storage/ice1/5/1/wsun377/ray_results_archive/054_mat_min_scratch_v2_512x512_20260419_185019/TeamVsBaselineShapingPPOTrainer_Soccer_31190_00000_0_2026-04-19_18-50-41/checkpoint_000970/checkpoint-970 vs baseline: win_rate=0.878 (878W-122L-0T)
/storage/ice1/5/1/wsun377/ray_results_archive/054_mat_min_scratch_v2_512x512_20260419_185019/TeamVsBaselineShapingPPOTrainer_Soccer_31190_00000_0_2026-04-19_18-50-41/checkpoint_001050/checkpoint-1050 vs baseline: win_rate=0.867 (867W-133L-0T)
/storage/ice1/5/1/wsun377/ray_results_archive/054_mat_min_scratch_v2_512x512_20260419_185019/TeamVsBaselineShapingPPOTrainer_Soccer_31190_00000_0_2026-04-19_18-50-41/checkpoint_001100/checkpoint-1100 vs baseline: win_rate=0.880 (880W-120L-0T)
/storage/ice1/5/1/wsun377/ray_results_scratch/054_mat_min_scratch_v2_512x512_20260420_092037/TeamVsBaselineShapingPPOTrainer_Soccer_c660d_00000_0_2026-04-20_09-21-01/checkpoint_001230/checkpoint-1230 vs baseline: win_rate=0.880 (880W-120L-0T)
/storage/ice1/5/1/wsun377/ray_results_scratch/054_mat_min_scratch_v2_512x512_20260420_092037/TeamVsBaselineShapingPPOTrainer_Soccer_c660d_00000_0_2026-04-20_09-21-01/checkpoint_001240/checkpoint-1240 vs baseline: win_rate=0.875 (875W-125L-0T)
[suite-parallel] total_elapsed=479.5s tasks=10 parallel=7
```
