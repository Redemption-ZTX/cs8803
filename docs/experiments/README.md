...
| [SNAPSHOT-079](snapshot-079-055v3-recursive-distill.md) | **055v3 Single-Teacher Recursive Distill from 055v2@1000**: 单 teacher (055v2) → 031B student。测 student capacity 不再是 binding 时, 是否仍能 +1.7pp 再现 055 pattern。 | 2026-04-21 | **§3.3 tied** (peak 0.914, +0.005 vs teacher 0.909, single-teacher recursive 在 saturated SOTA tier 上 marginal ≤ noise) |
| [SNAPSHOT-080](snapshot-080-pool-A-v2-with-1750-teacher.md) | **Pool A v2 with 1750-SOTA teacher swap**: 上调 Pool A teacher = {055v2_extend@1750, 055@1150, 056D@1140} 看是否能撬动 +0.005 marginal。 | 2026-04-21 | 训练中 |
| [SNAPSHOT-081](snapshot-081-aggressive-offense-reward.md) | **Aggressive-Offense Reward Scratch — Orthogonal Reward Family**: 全新 reward asymmetric 偏向进攻 (no defense, no possession, strong shot reward), scratch train 出 qualitatively different agent 当 ensemble member, 期望 baseline WR [0.75, 0.88]。 | 2026-04-21 | 训练中 (orthogonal specialist input for DIR-A Wave 2) |
| [SNAPSHOT-082](snapshot-082-hierarchical-two-stream-siamese.md) | **Hierarchical Two-Stream Siamese**: 架构变体 — 双 stream encoder (self / teammate stream), 上层 hierarchical merge。 | 2026-04-21 | 预注册 |
| [SNAPSHOT-083](snapshot-083-per-ray-attention.md) | **Per-Ray Attention**: 架构变体 — 每条 ray 独立 attention, 替代 Siamese encoder 的 global pool。 | 2026-04-21 | 预注册 |
| [SNAPSHOT-099](snapshot-099-stone-pipeline-strategic-synthesis.md) | **Stone-Pipeline Strategic Synthesis — 5/5 distill saturation 后的范式 pivot**: 综合 071/072/073/076/079 distill saturation 证据 + Stone lab 4 篇文献 (Veloso 2000 / Sutton 2005 / TPOT-RL 1998 / Wang Stone Hanna ICRA 2025) → 6-direction work plan (DIR-A heuristic selector / DIR-B layered training / DIR-E option-critic / DIR-F VDN / DIR-G MoE router / PIPELINE)。DIR-D-QMIX defer to BACKLOG。 | 2026-04-22 | 战略综述 (snapshot-only) |
| [SNAPSHOT-100](snapshot-100-dir-A-heuristic-selector.md) | **DIR-A 4-Phase Heuristic State-Conditional Selector (Stone Wave 1)**: hand-coded geometric phase classifier (NEAR_GOAL / BALL_DUEL / POSITIONING / MID_FIELD) routes per-step to 1 of K experts。Wave 1 mapping: NEAR/MID/POS=1750-SOTA, BALL_DUEL=055@1150。 | 2026-04-22 | **Wave 1 §3.4 sub-SOTA** (200ep 0.875, heuristic forced wrong specialist for high-freq BALL_DUEL phase); Wave 2 trigger pending 081 + 103-series ready |
| [SNAPSHOT-101](snapshot-101-dir-B-layered-phase1.md) | **DIR-B Layered Learning Phase 1 — Ball-Control Specialist (vs RANDOM only)**: Stone Veloso 2000 hierarchical training, Phase 1 = pure ball-control (031B arch + ball_progress 0.05 + possession_bonus 0.005, NO defense, NO shot, vs RANDOM only)。500 iter / 4h budget, output is PIPELINE Phase 2-5 frozen base。 | 2026-04-22 | 训练中 (jobid 5033289 atl1-1-03-017-23-0); verdict pending Stage 1 post-eval ~ETA 4h |
| [SNAPSHOT-102](snapshot-102-dir-F-vdn.md) | **DIR-F VDN — Sunehag 2017 Decomposed Critic on 031B Cross-Attention**: joint V = V_0(s_0) + V_1(s_1) + bias, each V_i 只看 agent i 的 encoder feature, no cross-attention; actor 仍 centralized。1250 iter / 12h, scratch, v2 reward (isolate VDN as only variable)。 | 2026-04-22 | 训练中 (jobid 5032914 atl1-1-03-015-30-0); verdict pending Stage 1 post-eval ~ETA 12h |
| [SNAPSHOT-103](snapshot-103-dir-A-wave3-sub-task-specialists.md) | **DIR-A Wave 3 Sub-Task Specialist Lanes (103A INTERCEPTOR / 103B DEFENDER / 103C MID-FIELD-DRIBBLE)**: 各自 scenario init + reward shape 在 BALL_DUEL / POSITIONING / MID_FIELD phase 上 dominant 的 sub-task specialists。Wave 1 = scenario + reward only; Wave 2 BACKLOG = + state masking + action constraint。 | 2026-04-22 | 预注册 / 待 env extension + launchers + 12h GPU 并行 |
| [SNAPSHOT-104](snapshot-104-dir-G-moe-router.md) | **DIR-G MoE Router over Frozen Experts (Wave 1 Uniform + Wave 2 REINFORCE)**: K experts + per-step hard switch (vs v074 family 的 prob averaging)。Wave 1 = uniform random. Wave 2 = REINFORCE-trained router NN。 | 2026-04-22 | **Wave 1 §3.1 HIT** (200ep 0.900, uniform > DIR-A heuristic biased-toward-weak); Wave 2 router NN training pending |
| [SNAPSHOT-105](snapshot-105-dir-E-option-critic-frozen.md) | **DIR-E Option-Critic with Frozen Intra-Option Policies (Wave 1 Random NN + Wave 2 REINFORCE)**: Bacon 2017 OC with frozen K experts as intra-option policies, only train termination β + selector NN。Wave 1 = random-init NN; Wave 2 = REINFORCE + entropy reg on β to prevent collapse。 | 2026-04-22 | **Wave 1 §3.1 HIT** (200ep 0.895, ≈ DIR-G within SE); Wave 2 termination/selector NN training pending |
| [SNAPSHOT-108](snapshot-108-stone-layered-l2-improvement-paths.md) | **103A-wd v2 Mechanism Analysis + Stone Layered L2 Improvement Paths**: 解释 BUG-1/BUG-2 修复后 v2 @400 0.920 (+0.007 over v1) / 终段 +0.022 来源 (anchor schedule 主导, opp resample 次要); 5 路径 P1-P5 ranked by EV/cost (P1 longer training / P3 multi-scenario / P2 α sweep / P4 ensemble teacher / P5 recursive); 推荐 P1 first + P3 second, defer P4/P5。 | 2026-04-23 | 路径分析 + 路径预注册 (后续 lane 触发) |
