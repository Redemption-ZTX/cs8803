...
CS8803 深度强化学习课程 Final Project — 基于
[soccer-twos starter kit](https://github.com/mdas64/soccer-twos-starter)
训练的 2v2 足球多智能体。
...
提交的策略 `055v2_extend@1750` 是单次前向推理的 agent，由 3 阶段 recursive distillation pipeline 训练得到 (Born-Again-Networks 风格):

1. **Stage 1** — 把 3-teacher deploy-time ensemble (`031B + 045A + 051A`) 蒸馏到一个新 student。
2. **Stage 2** — 用 Stage 1 的 student + 一个 LR-diverse variant 刷新 teacher pool, 再蒸馏一次。
3. **Stage 3** — 从原 iter-1250 horizon 继续训练到 iter 2000, 选择 variance 最低的 checkpoint。

我们自己的 9000-episode 复评进一步确认 vs. Baseline 的 WR 是 `0.907 ± 0.004` (CI `[0.899, 0.914]`)。
...
| 层级 | 锁定版本 | 说明 |
|---|---|---|
| Python | **3.8** (严格) | Ray RLlib 1.4 / ML-Agents 0.27 强制要求 |
| PyTorch | 2.x | 所有训练脚本默认使用 PyTorch |
| Ray RLlib | **1.4.0** (严格) | 单团队 PPO + 多 agent 后端 |
| Unity ML-Agents | 0.27.0 | 以 headless 方式运行 Soccer-Twos binary |
| `protobuf` | **3.20.3** (严格) | 兼容性锁定 |
| `pydantic` | **1.10.13** (严格) | 兼容性锁定 |
| 计算资源 | NVIDIA H100 (PACE) | 长训练全部走 SLURM |
| 报告 | Tectonic + HTML/SVG | LaTeX `main.pdf` + Headless Chrome 渲染图表 |

> Python 和 Ray **不要升级** — 上游兼容性硬性要求。
...
├── report/                              # 最终报告 LaTeX 源 + 编译产物
├── submission/                          # 课程提交包（CS8803DRL_AGENT.zip + 解压目录）
└── docs/                                # 项目文档
```
