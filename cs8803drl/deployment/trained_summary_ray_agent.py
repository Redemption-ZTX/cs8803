...
from cs8803drl.branches.obs_summary import SUMMARY_POLICY_ID, append_ray_summary_features, build_summary_obs_space
...
ALGORITHM = "PPO"
_DUMMY_ENV_NAME = f"DummyEnvTrainedSummaryRayAgent_{os.getpid()}"
...
def _default_checkpoint_path():
    for env_name in ("TRAINED_RAY_CHECKPOINT", "TRAINED_SUMMARY_RAY_CHECKPOINT"):
...
class SummaryRayAgent(AgentInterface):
    def __init__(self, env: gym.Env):
        ...
        summary_obs_space = build_summary_obs_space(obs_space)
        ...
        tune.registry.register_env(
            _DUMMY_ENV_NAME, lambda *_: _DummyGymEnv(summary_obs_space, act_space)
        )
        ...
        self.policy = load_policy_weights(checkpoint_path, trainer, SUMMARY_POLICY_ID)

    def act(self, observation: Dict[int, np.ndarray]) -> Dict[int, np.ndarray]:
        actions = {}
        for player_id, obs in observation.items():
            summary_obs = append_ray_summary_features(obs)
            action = _unwrap_action(self.policy.compute_single_action(summary_obs))
