...
_DUMMY_ENV_NAME = "DummyEnvTrainedTeamRayAgent"
...
def _default_checkpoint_path():
    ...
    if checkpoint:
        return checkpoint
    raise ValueError("Missing TRAINED_RAY_CHECKPOINT env var for team-level policy.")
...
def _build_team_action_space(raw_action_space):
    if not isinstance(raw_action_space, gym.spaces.MultiDiscrete):
        raise ValueError(
            "trained_team_ray_agent expects env.action_space to be MultiDiscrete, "
...
def _normalize_team_action(action, expected_dim):
    ...
    raise ValueError(
        f"Team policy returned action with shape {arr.shape}, expected {expected_dim} values."
    )


class TeamRayAgent(AgentInterface):
    ...
    def act(self, observation: Dict[int, np.ndarray]) -> Dict[int, np.ndarray]:
        ...
        if len(player_ids) != 2:
            raise ValueError(
                "trained_team_ray_agent expects exactly 2 local teammates in observation, "
