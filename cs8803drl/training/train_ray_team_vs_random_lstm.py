...
os.environ.setdefault("EVAL_TEAM0_MODULE", "cs8803drl.deployment.trained_lstm_ray_agent")
...
from cs8803drl.branches.lstm_transfer import warmstart_lstm_policy
...
DEFAULT_TIME_TOTAL_S = 21_600
DEFAULT_RUN_NAME = "PPO_team_lstm"
DEFAULT_NUM_GPUS = 1
...
def _after_init_warmstart(trainer):
    ...
    stats = warmstart_lstm_policy(trainer, warmstart_path, policy_name="default_policy")
    _console_print(
        "[warmstart] copied source feed-forward weights into LSTM policy "
        f"(copied={stats['copied']}, skipped={stats['skipped']})"
    )


LSTMPPOTrainer = PPOTrainer.with_updates(
    name="LSTMPPOTrainer",
...
def main():
    ...
    sgd_minibatch_size_default = min(2048, max(512, train_batch_size // 8))
    ...
    fcnet_hiddens_raw = os.environ.get("FCNET_HIDDENS", "").strip()
    fcnet_hiddens = [int(piece.strip()) for piece in fcnet_hiddens_raw.split(",") if piece.strip()] if fcnet_hiddens_raw else list(DEFAULT_FCNET_HIDDENS)
    fcnet_activation = os.environ.get("FCNET_ACTIVATION", "relu").strip() or "relu"
    use_reward_shaping = _env_bool("USE_REWARD_SHAPING", False)
    lstm_cell_size = _env_int("LSTM_CELL_SIZE", DEFAULT_LSTM_CELL_SIZE)
    ...
    if warmstart_path and not _env_bool("ALLOW_LSTM_RESTORE", False):
        restore_path = None
    if restore_path:
    ...
    _console_print("LSTM Configuration")
    _console_print(f"  warmstart_checkpoint: {warmstart_path or 'None'}")
    _console_print(f"  lstm_cell_size:       {lstm_cell_size}")
    _console_print(f"  max_seq_len:         {max_seq_len}")
