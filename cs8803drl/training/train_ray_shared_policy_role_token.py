...
os.environ.setdefault("EVAL_TEAM0_MODULE", "cs8803drl.deployment.trained_shared_role_agent")
...
from cs8803drl.branches.shared_role_token import (
    SHARED_POLICY_ID,
    build_role_token_map,
    shared_policy_mapping_fn,
    team0_agent_ids,
    warmstart_shared_policy,
)
...
DEFAULT_TIMESTEPS_TOTAL = 2_000_000
DEFAULT_TIME_TOTAL_S = 7_200
DEFAULT_RUN_NAME = "PPO_shared_role_token"
...
def _after_init_warmstart(trainer):
    ...
    stats = warmstart_shared_policy(trainer, warmstart_path, policy_id=SHARED_POLICY_ID)
    _console_print(
        "[warmstart] copied source weights into "
        f"{SHARED_POLICY_ID} (copied={stats['copied']}, adapted={stats['adapted']}, skipped={stats['skipped']})"
    )


SharedRoleTokenPPOTrainer = PPOTrainer.with_updates(
    name="SharedRoleTokenPPOTrainer",
...
def main():
    ...
    use_reward_shaping = _env_bool("USE_ROLE_REWARD_SHAPING", False)
    ...
    if warmstart_path and not _env_bool("ALLOW_SHARED_RESTORE", False):
        restore_path = None
    if restore_path:
    ...
    base_env_config = {
        ...
        "role_token_obs": build_role_token_map(),
    }
    ...
    if use_reward_shaping:
        from cs8803drl.branches.role_specialization import build_role_reward_shaping_config

        env_config["reward_shaping"] = build_role_reward_shaping_config()
    ...
    config = {
        ...
        "multiagent": {
            "policies": {
                SHARED_POLICY_ID: (None, obs_space, act_space, {}),
                BASELINE_POLICY_ID: (
                    FrozenBaselinePolicy,
                    obs_space,
                    act_space,
                    {"strip_obs_tail_dims": ROLE_TOKEN_DIM},
                ),
            },
            "policy_mapping_fn": tune.function(shared_policy_mapping_fn),
    ...
    _console_print("Shared Policy Configuration")
    _console_print(f"  team0_agent_ids:      {team0_agent_ids()}")
    _console_print(f"  warmstart_checkpoint: {warmstart_path or 'None'}")
    _console_print(f"  role_token_obs:       enabled (+{ROLE_TOKEN_DIM} dims)")
