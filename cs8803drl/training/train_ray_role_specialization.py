...
os.environ.setdefault("EVAL_TEAM0_MODULE", "cs8803drl.deployment.trained_role_agent")
...
from cs8803drl.branches.role_specialization import (
    ...
    defender_agent_id,
    role_policy_mapping_fn,
    warmstart_role_policies,
)
...
DEFAULT_TIMESTEPS_TOTAL = 5_000_000
DEFAULT_TIME_TOTAL_S = 7_200
DEFAULT_RUN_NAME = "PPO_role_specialized"
...
DEFAULT_NUM_SGD_ITER = 5
DEFAULT_BASE_PORT = 64_105
DEFAULT_LOG_LEVEL = "WARN"
DEFAULT_FCNET_HIDDENS = (256, 128)


def _parse_role_policies_to_train(raw: str):
    raw = (raw or "both").strip().lower()
    mapping = {
        "both": [ATTACKER_POLICY_ID, DEFENDER_POLICY_ID],
        "attacker": [ATTACKER_POLICY_ID],
        "defender": [DEFENDER_POLICY_ID],
    }
    if raw in mapping:
        return mapping[raw]

    pieces = [piece.strip().lower() for piece in raw.split(",") if piece.strip()]
    selected = []
    for piece in pieces:
        if piece == "attacker":
            selected.append(ATTACKER_POLICY_ID)
        elif piece == "defender":
            selected.append(DEFENDER_POLICY_ID)
        else:
            raise ValueError(
                "ROLE_POLICIES_TO_TRAIN must be one of: both, attacker, defender, "
                f"or a comma-separated subset. Got: {raw}"
            )
    if not selected:
        raise ValueError("ROLE_POLICIES_TO_TRAIN selected no trainable policies.")
    return selected


def _validate_base_port(name, base_port, *, worker_slots):
    base_port = int(base_port)
    max_port = 65535
    required_max = base_port + max(0, int(worker_slots))
    if base_port < 1024 or required_max > max_port:
        raise ValueError(
            f"{name}={base_port} is invalid for {worker_slots} worker slots; "
            f"choose a base port between 1024 and {max_port - int(worker_slots)}."
        )
    return base_port
...
def _after_init_warmstart(trainer):
    ...
    warmstart_role_policies(trainer, warmstart_path)
    _console_print(f"[warmstart] copied source weights into {ATTACKER_POLICY_ID}/{DEFENDER_POLICY_ID}")


RoleSpecializedPPOTrainer = PPOTrainer.with_updates(
    name="RoleSpecializedPPOTrainer",
...
def main():
    ...
    sgd_minibatch_size_default = min(2048, max(1024, train_batch_size // 8))
    ...
    use_role_reward_shaping = _env_bool("USE_ROLE_REWARD_SHAPING", True)
    ...
    policies_to_train = _parse_role_policies_to_train(
        os.environ.get("ROLE_POLICIES_TO_TRAIN", "both")
    )
    ...
    if warmstart_path and not _env_bool("ALLOW_ROLE_RESTORE", False):
        restore_path = None
    if restore_path:
    ...
    base_port = _validate_base_port(
        "BASE_PORT",
        _env_int("BASE_PORT", DEFAULT_BASE_PORT),
        worker_slots=(num_workers * num_envs_per_worker + 8),
    )
    ...
    reward_shaping = build_role_reward_shaping_config() if use_role_reward_shaping else None
    ...
    if reward_shaping:
        env_config["reward_shaping"] = reward_shaping
    ...
    config = {
        ...
        "multiagent": {
            "policies": {
                ATTACKER_POLICY_ID: (None, obs_space, act_space, {}),
                DEFENDER_POLICY_ID: (None, obs_space, act_space, {}),
                BASELINE_POLICY_ID: (FrozenBaselinePolicy, obs_space, act_space, {}),
            },
            "policy_mapping_fn": tune.function(role_policy_mapping_fn),
    ...
    _console_print(f"Role Configuration")
    _console_print(f"  attacker_agent_id:    {attacker_id}")
    _console_print(f"  defender_agent_id:    {defender_id}")
    _console_print(f"  warmstart_checkpoint: {warmstart_path or 'None'}")
    _console_print(
        f"  reward_shaping:       {'enabled' if use_role_reward_shaping else 'disabled'}"
    )
    _console_print(f"  policies_to_train:    {', '.join(policies_to_train)}")
