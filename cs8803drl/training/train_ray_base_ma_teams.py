...
os.environ.setdefault("EVAL_TEAM0_MODULE", "cs8803drl.deployment.trained_ma_team_agent")
...
DEFAULT_RUN_NAME = "PPO_base_ma_teams"
...
DEFAULT_BASE_PORT = 5_205
DEFAULT_LOG_LEVEL = "INFO"


BaseMATeamsPPOTrainer = PPOTrainer.with_updates(
    name="BaseMATeamsPPOTrainer",
)
...
def _maybe_model_hiddens():
    raw = os.environ.get("FCNET_HIDDENS", "").strip()
    if not raw:
        return None
    return _env_layers("FCNET_HIDDENS", ())
...
def _print_base_header(
    *,
    ...
    _console_print(f"  model_hidden:         {fcnet_hiddens if fcnet_hiddens is not None else 'default (starter)'}")
    _console_print(f"  env_variation:        {EnvType.multiagent_team}")
    _console_print("  starter_alignment:    example_ray_ma_teams.py")
    _console_print("  reward_shaping:       disabled")
    _console_print("  training_mode:        scratch base-model lane")
    _console_print("  policy_structure:     shared team-level policy")
...
def main():
    ...
    fcnet_hiddens = _maybe_model_hiddens()
    ...
    config = {
        ...
        "multiagent": {
            ...
            "policy_mapping_fn": tune.function(lambda _agent_id: "default"),
