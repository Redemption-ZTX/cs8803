...
DEFAULT_RUN_NAME = "PPO_shared_central_critic"
...
SharedCCPPOTrainer = PPOTrainer.with_updates(
    name="SharedCCPPOTrainer",
...
def main():
    ...
    if warmstart_path and not _env_bool("ALLOW_CC_RESTORE", False):
        restore_path = None
    if restore_path:
    ...
    _console_print("Centralized Critic Configuration")
    _console_print(f"  warmstart_checkpoint: {warmstart_path or 'None'}")
    _console_print("  actor_policy:         shared team0 actor")
