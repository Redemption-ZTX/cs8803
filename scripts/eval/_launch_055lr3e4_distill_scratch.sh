#!/bin/bash
# 1a (Tier 1): 055 + lr=3e-4 combo — ensemble distill with 056D's winning LR.
# Same as 055 distill 034E ensemble, only change: LR=0.0001 → LR=0.0003.
# Hypothesis: distill + high LR stack → break 0.91 combined 2000ep ceiling.
# See snapshot-055 §8 Outcome A follow-up path.
set -euo pipefail
...
# PORT_SEED=41 — free anchor (054=31, 053A=23, 051D=51, 031B-noshape=13, 055=37, 053Dmirror=33)
PORT_SEED=${PORT_SEED:-41}
PORT_OFFSET=$(( (PORT_SEED % 60) * 50 ))

export RUN_NAME=055lr3e4_distill_034e_ensemble_to_031B_scratch_$(date +%Y%m%d_%H%M%S)
export RAY_ADDRESS_OVERRIDE=local
export RAY_SESSION_TMPDIR_OVERRIDE=/tmp/r055lr3e4_${PORT_SEED}
...
# Distillation from 034E ensemble (3 team-level teachers, avg factor probs) — same as 055
...
# *** Key difference from 055: LR=3e-4 (from 056D winning config) ***
export LR=0.0003 CLIP_PARAM=0.15

export BASELINE_PROB=1.0

# v2 shaping (same as 031B / 055)
...
# Budget: same as 055 (1250 iter scratch, 50M steps, ~12h)
...
LOG=docs/experiments/artifacts/slurm-logs/055lr3e4_distill_train_$(date +%Y%m%d_%H%M%S).log
