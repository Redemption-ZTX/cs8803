...
LANE_TAG=104A_layered_p2_passdecision_restore
...
# Restore from 104A ckpt 340 (last good ckpt before SLURM kill at iter 344)
TRIAL=$(ls -d /storage/ice1/5/1/wsun377/ray_results_scratch/104A_layered_p2_passdecision_*/TeamVsBaselineShapingPPOTrainer_Soccer_*/ | head -1)
TRIAL=${TRIAL%/}
export RESTORE_CHECKPOINT=$TRIAL/checkpoint_000340/checkpoint-340
...
export RUN_NAME=104A_layered_p2_passdecision_restore_$(date +%Y%m%d_%H%M%S)
export RAY_ADDRESS_OVERRIDE=local
export RAY_SESSION_TMPDIR_OVERRIDE=/tmp/r104Ares_${PORT_SEED}
...
export SHAPING_EVENT_PASS_REWARD=0.05
...
LOG=docs/experiments/artifacts/slurm-logs/104A_restore_train_$(date +%Y%m%d_%H%M%S).log
