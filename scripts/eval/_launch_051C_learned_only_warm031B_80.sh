#!/bin/bash
# 051C: learned-only (no v2) on 031B cross-attention warmstart from EARLY ckpt 80 (50ep WR=0.50).
...
#   - 051C learned-only on 031B@80 (early, weak baseline)
# Hypothesis: 051C 1000ep peak ≫ 0.50 → learned reward CAN drive incremental improvement
...
export RUN_NAME=051C_learned_only_warm031B80_cross_attention_512x512_$(date +%Y%m%d_%H%M%S)
export RAY_ADDRESS_OVERRIDE=local
export RAY_SESSION_TMPDIR_OVERRIDE=/tmp/r51C_${PORT_SEED}
...
LOG=docs/experiments/artifacts/slurm-logs/051C_train_$(date +%Y%m%d_%H%M%S).log
