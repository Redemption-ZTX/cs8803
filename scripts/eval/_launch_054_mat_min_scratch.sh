#!/bin/bash
# 054: MAT-min scratch — 031B (cross-attn within agent) + cross-AGENT attention residual.
# Strict avoid 052 pitfalls: NO FFN, NO LayerNorm, NO merge change. Just cross-agent
# attention block (~98K extra params, ~21% increase vs 031B 0.46M).
# V proj zero-init → graceful degrade to 031B at start of training.
#
# Hypothesis: explicit pairwise agent interaction (MAT-style) > implicit (031B concat merge).
...
unset TEAM_TRANSFORMER TEAM_TRANSFORMER_MIN TEAM_TRANSFORMER_MHA

export OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1

# Hard-coded PORT_SEED far from active lanes (053A=23, 051D=51, 031B-noshape=13)
PORT_SEED=${PORT_SEED:-31}
PORT_OFFSET=$(( (PORT_SEED % 60) * 50 ))

export RUN_NAME=054_mat_min_scratch_v2_512x512_$(date +%Y%m%d_%H%M%S)
export RAY_ADDRESS_OVERRIDE=local
export RAY_SESSION_TMPDIR_OVERRIDE=/tmp/r054_${PORT_SEED}
export NUM_GPUS=1 NUM_WORKERS=8 NUM_ENVS_PER_WORKER=5
export FCNET_HIDDENS=512,512

# Architecture: 031B base + NEW cross-agent attention
...
export TEAM_CROSS_AGENT_ATTN=1
export TEAM_CROSS_AGENT_ATTN_DIM=64
...
LOG=docs/experiments/artifacts/slurm-logs/054_mat_min_train_$(date +%Y%m%d_%H%M%S).log
