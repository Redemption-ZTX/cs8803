...
export RUN_NAME=071_poolA_homogeneous_distill_warm031B80_$(date +%Y%m%d_%H%M%S)
export RAY_ADDRESS_OVERRIDE=local
export RAY_SESSION_TMPDIR_OVERRIDE=/tmp/r071_${PORT_SEED}
...
LOG=docs/experiments/artifacts/slurm-logs/071_poolA_train_$(date +%Y%m%d_%H%M%S).log
echo "[$(date)] 071 Pool A launching: 3-teacher homogeneous distill, BASE_PORT=$BASE_PORT" >&2
