#!/bin/bash
# 057: RND (Random Network Distillation, Burda 2019) intrinsic motivation on 031B arch.
# Per-step intrinsic = ||target(obs) - predictor(obs)||^2 normalized + clipped.
# Hypothesis: novel-state exploration bonus pushes peak above 031B 0.880 baseline.
# See snapshot-057-rnd-intrinsic-motivation.md for full design + simplification risks.
set -euo pipefail
...
# PORT_SEED far from active lanes (054=31, 055=37, 053Acont=25, 056ABCD=41-44)
PORT_SEED=${PORT_SEED:-35}
PORT_OFFSET=$(( (PORT_SEED % 60) * 50 ))

export RUN_NAME=057_rnd_scratch_v2_512x512_$(date +%Y%m%d_%H%M%S)
export RAY_ADDRESS_OVERRIDE=local
export RAY_SESSION_TMPDIR_OVERRIDE=/tmp/r057_${PORT_SEED}
...
# RND intrinsic motivation (snapshot-057 §2.2)
export RND_ENABLED=1
export RND_HIDDEN_DIM=256
export RND_EMBED_DIM=64
export RND_INTRINSIC_BETA=0.01      # initial β; equal to v2 ball_progress scale
export RND_LR=0.0001                # predictor optimizer LR
export RND_TRAIN_EVERY_STEPS=16     # predictor update every 16 env steps
export RND_TRAIN_BATCH_SIZE=256
export RND_WARMUP_STEPS=10000       # skip first 10K env steps (predictor needs warmup)
export RND_RANDOM_SEED=1234         # deterministic target across workers
...
# v2 shaping ON (RND is additive on top of v2)
...
# Budget: 1250 iter scratch (same as 031B / 054 / 055)
...
LOG=docs/experiments/artifacts/slurm-logs/057_rnd_train_$(date +%Y%m%d_%H%M%S).log
