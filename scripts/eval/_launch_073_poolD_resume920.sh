#!/bin/bash
# 071 Pool A: Homogeneous distill family 3-teacher ensemble {055@1150 + 055v2@1000 + 056D@1140}
# Student: 031B arch warm from 031B@80 (matches 053D/068 warm-start pattern)
# See snapshot-071-pool-A-homogeneous-distill.md for full design + H_071 hypothesis (≥ 0.915).
# Using 055v2@1000 pre-extend (combined 3000ep 0.909 verified) as T2 per user's blocker-semantics
# clarification 06:45 — launch now, upgrade later if 055v2_extend gives better ckpt.
set -euo pipefail
export RESTORE_CKPT_PATH=/storage/ice1/5/1/wsun377/ray_results_scratch/073_poolD_cross_reward_warm031B80_20260421_113510/TeamVsBaselineShapingPPOTrainer_Soccer_baeef_00000_0_2026-04-21_11-35-31/checkpoint_000920/checkpoint-920
cd /home/hice1/wsun377/Desktop/cs8803drl

LANE_TAG=073_poolD_resume
...
export RESTORE_CHECKPOINT="/storage/ice1/5/1/wsun377/ray_results_scratch/073_poolD_cross_reward_warm031B80_20260421_113510/TeamVsBaselineShapingPPOTrainer_Soccer_baeef_00000_0_2026-04-21_11-35-31/checkpoint_000920/checkpoint-920"
...
PORT_SEED=${PORT_SEED:-73}
PORT_OFFSET=$(( (PORT_SEED % 60) * 50 ))

export RUN_NAME=073_poolD_resume920_to_1250_$(date +%Y%m%d_%H%M%S)
export RAY_ADDRESS_OVERRIDE=local
export RAY_SESSION_TMPDIR_OVERRIDE=/tmp/r073_${PORT_SEED}
...
# Warm-start from 031B@80 (per snapshot-071 §2.5)

# Pool D: cross-reward 3-teacher (055 v2+distill, 068_warm PBRS+distill, 062a no-shape+curric)
export TEAM_DISTILL_ENSEMBLE_KL=1
export TEAM_DISTILL_TEACHER_ENSEMBLE_CHECKPOINTS="/storage/ice1/5/1/wsun377/ray_results_scratch/055_distill_034e_ensemble_to_031B_scratch_20260420_092037/TeamVsBaselineShapingPPOTrainer_Soccer_c6b12_00000_0_2026-04-20_09-21-01/checkpoint_001150/checkpoint-1150,/storage/ice1/5/1/wsun377/ray_results_scratch/068_055PBRS_distill_warm031B80_20260421_041448/TeamVsBaselineShapingPPOTrainer_Soccer_36a5c_00000_0_2026-04-21_04-15-10/checkpoint_000780/checkpoint-780,/storage/ice1/5/1/wsun377/ray_results_scratch/062a_curriculum_noshape_adaptive_0_200_500_1000_20260420_142908/TeamVsBaselineShapingPPOTrainer_Soccer_dd9fa_00000_0_2026-04-20_14-29-28/checkpoint_001220/checkpoint-1220"
...
# Budget: warm-start saves some iters vs scratch but keep same budget for safety
...
LOG=docs/experiments/artifacts/slurm-logs/073_poolD_train_$(date +%Y%m%d_%H%M%S).log
echo "[$(date)] 073 Pool D launching: 3-teacher homogeneous distill, BASE_PORT=$BASE_PORT" >&2
