...
PORT_SEED=${PORT_SEED:-80}
PORT_OFFSET=$(( (PORT_SEED % 60) * 50 ))

export RUN_NAME=080_poolAv2_with_1750_teacher_warm031B80_$(date +%Y%m%d_%H%M%S)
export RAY_ADDRESS_OVERRIDE=local
export RAY_SESSION_TMPDIR_OVERRIDE=/tmp/r080_${PORT_SEED}
...
LOG=docs/experiments/artifacts/slurm-logs/080_poolAv2_train_$(date +%Y%m%d_%H%M%S).log
echo "[$(date)] 080 Pool A v2 (with 1750 SOTA teacher) launching: 3-teacher homogeneous distill, BASE_PORT=$BASE_PORT" >&2
