#!/bin/bash
# 103A: Stone DIR-A Wave 3 — INTERCEPTOR sub-task specialist (BALL_DUEL phase).
# Wang/Stone/Hanna ICRA 2025 SPL 2024 winner: trained 4 sub-policies with
# scenario init + own state/action/reward. We replicate Wave 1 with
# scenario init + reward shape only (state mask + action constraint = Wave 2 BACKLOG).
# Scenario: opp very close to ball (within 0.6m), our agent close-by → must duel for ball.
# Reward: tackle 0.10, clearance 0.05, opp_progress_penalty 0.05 (steal & push back).
set -euo pipefail
cd /home/hice1/wsun377/Desktop/cs8803drl

LANE_TAG=103A_interceptor
...
PORT_SEED=${PORT_SEED:-103}
PORT_OFFSET=$(( (PORT_SEED % 60) * 50 ))

export RUN_NAME=103A_interceptor_$(date +%Y%m%d_%H%M%S)
export RAY_ADDRESS_OVERRIDE=local
export RAY_SESSION_TMPDIR_OVERRIDE=/tmp/r103A_${PORT_SEED}
...
# Sub-task scenario init: BALL_DUEL phase
export SCENARIO_RESET=interceptor_subtask
# Vs random opponent (no baseline) — focus on the BALL_DUEL skill, not full game
export BASELINE_PROB=0.0

# INTERCEPTOR reward shape: tackle + clearance + steal-related events
...
export SHAPING_OPP_PROGRESS_PENALTY=0.05      # high — prevent opp from advancing
export SHAPING_POSSESSION_BONUS=0.005         # encourage stealing ball
...
export SHAPING_EVENT_TACKLE_REWARD=0.10       # main reward
export SHAPING_EVENT_CLEARANCE_REWARD=0.05    # secondary
export SHAPING_EVENT_COOLDOWN_STEPS=5

# Budget: smaller (sub-task simpler than full 2v2)
...
LOG=docs/experiments/artifacts/slurm-logs/103A_interceptor_train_$(date +%Y%m%d_%H%M%S).log
