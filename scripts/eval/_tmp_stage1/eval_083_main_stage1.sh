...
CKPTS="460 470 480 510 520 530 550 560 570 580 600 610 620 650 660 670 680 690 700 750 760 770"
ARGS=""
for c in $CKPTS; do
  ARGS="$ARGS --checkpoint $TRIAL/checkpoint_$(printf %06d $c)/checkpoint-$c"
done
mkdir -p docs/experiments/artifacts/official-evals/parallel-logs/083_baseline1000
exec /home/hice1/wsun377/.venvs/soccertwos_h100/bin/python scripts/eval/evaluate_official_suite_parallel.py \
  --team0-module cs8803drl.deployment.trained_team_ray_agent \
  --opponents baseline \
  -n 1000 -j 7 \
  --base-port 62305 \
  --save-logs-dir docs/experiments/artifacts/official-evals/parallel-logs/083_baseline1000 \
  $ARGS \
  2>&1 | tee docs/experiments/artifacts/official-evals/083_baseline1000.log
