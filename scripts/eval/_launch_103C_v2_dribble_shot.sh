#!/bin/bash
# 103C: Stone DIR-A Wave 3 — MID-FIELD-DRIBBLE sub-task specialist.
# Scenario: mid field neutral, controlled player close to ball, no immediate opp pressure.
# Reward: ball_progress 0.05, possession 0.005, NO shot, NO event reward.
# Goal: dribble forward + hold possession, save shooting for the dedicated NEAR-GOAL specialist (081).
# Differs from 101A (Layered Phase 1 ball-control): 103C runs in NORMAL 2v2 env with
# scenario init biased to "I have ball mid-field neutral", while 101A is in random-opponent
# env without scenario init. 103C is closer to PIPELINE Phase 2 sub-behavior; 101A is Phase 1 base.
set -euo pipefail
cd /home/hice1/wsun377/Desktop/cs8803drl

LANE_TAG=103C_v2_dribble_shot
...
PORT_SEED=${PORT_SEED:-153}
PORT_OFFSET=$(( (PORT_SEED % 60) * 50 ))

export RUN_NAME=103C_v2_dribble_shot_$(date +%Y%m%d_%H%M%S)
export RAY_ADDRESS_OVERRIDE=local
export RAY_SESSION_TMPDIR_OVERRIDE=/tmp/r103Cv2_${PORT_SEED}
...
# Sub-task scenario: MID-FIELD-DRIBBLE phase
export SCENARIO_RESET=dribble_subtask
export BASELINE_PROB=0.0

# DRIBBLE reward shape: ball_progress + possession, NO shot reward
export USE_REWARD_SHAPING=1
export SHAPING_FIELD_ROLE_BINDING=0
export SHAPING_TIME_PENALTY=0.001
export SHAPING_BALL_PROGRESS=0.05            # main — push ball toward opp goal
export SHAPING_GOAL_PROXIMITY_SCALE=0.0
export SHAPING_OPP_PROGRESS_PENALTY=0.0      # don't punish (passive when not in possession)
export SHAPING_POSSESSION_BONUS=0.005        # encourage holding ball
export SHAPING_POSSESSION_DIST=1.25
export SHAPING_PROGRESS_REQUIRES_POSSESSION=1  # only credit progress while in possession
...
export SHAPING_EVENT_SHOT_REWARD=0.02         # v2: small shot reward (allow occasional finish, not dominant vs ball_progress 0.05)
...
LOG=docs/experiments/artifacts/slurm-logs/103C_v2_dribble_shot_train_$(date +%Y%m%d_%H%M%S).log
