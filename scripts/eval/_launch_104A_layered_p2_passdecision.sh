#!/bin/bash
# 104A: Stone Layered Learning Phase 2 — pass-decision specialist (snapshot-107).
#
# Recipe (after today's BUG-1/BUG-2 fixes + Stone L2 lessons):
# (1) WARMSTART_CHECKPOINT=101A@460 (Layer 1 ball-control specialist, 0.851 baseline-competent ≥0.85 threshold)
# (2) KL distill anchor from 101A@460 with α 0.05→0.005 over 39000 updates (BUG-2 fix lesson — anchor throughout)
# (3) SCENARIO_RESET=pass_subtask (NEW snapshot-107) — 2 our agents 6-12m apart, ball with one, opp pressuring
# (4) BASELINE_PROB=0.7 (BUG-1 fix in core code automatic per-episode resample)
# (5) NEW pass event reward 0.05 — main Phase 2 specialty signal
# Plus v2 base shaping (ball_progress / opp_progress / possession).
#
# Hypothesis: Stone Layered Phase 2 is PARADIGM-GENERALIZABLE beyond INTERCEPTOR (103A-warm-distill@400 = 0.914).
# Expected combined 2000ep ≥ 0.92 if generalizes (above 1750 true 0.9066 + above 103A-wd 0.914).
set -euo pipefail
cd /home/hice1/wsun377/Desktop/cs8803drl

LANE_TAG=104A_layered_p2_passdecision
...
# (3) Scenario init — focus learning on pass-decision state distribution
export SCENARIO_RESET=pass_subtask
...
PORT_SEED=${PORT_SEED:-247}
PORT_OFFSET=$(( (PORT_SEED % 60) * 50 ))

export RUN_NAME=104A_layered_p2_passdecision_$(date +%Y%m%d_%H%M%S)
export RAY_ADDRESS_OVERRIDE=local
export RAY_SESSION_TMPDIR_OVERRIDE=/tmp/r104A_${PORT_SEED}
...
# (5) Reward — v2 base + NEW pass event reward
...
export SHAPING_OPP_PROGRESS_PENALTY=0.02    # mild defense
...
export SHAPING_EVENT_TACKLE_REWARD=0.0      # not pass specialty
export SHAPING_EVENT_CLEARANCE_REWARD=0.0
export SHAPING_EVENT_PASS_REWARD=0.05       # NEW — pass-decision specialty signal
export SHAPING_PASS_MIN_BALL_SPEED=1.0      # ball must move (not just possession ambiguity)
...
LOG=docs/experiments/artifacts/slurm-logs/104A_layered_p2_passdecision_train_$(date +%Y%m%d_%H%M%S).log
