#!/bin/bash
# SNAPSHOT-063 / 055-temp:
# - same 034E -> 031B ensemble distillation stack as 055
# - isolate only temperature change: T=1.0 -> T=2.0
# - test whether softer teacher distributions improve single-model distillation
# See snapshot-063-055-temp-sharpening.md for the full prereg.
set -euo pipefail
cd /home/hice1/wsun377/Desktop/cs8803drl

# Running-flag convention (same style as other launchers; direct execution supported)
LANE_TAG=063_T40
...
export RESTORE_CHECKPOINT="/storage/ice1/5/1/wsun377/ray_results_scratch/063_temp_distill_T40_on_034e_ensemble_031B_20260420_214137/TeamVsBaselineShapingPPOTrainer_Soccer_53a2f_00000_0_2026-04-20_21-42-16/checkpoint_000370/checkpoint-370"
...
PORT_SEED=${PORT_SEED:-99}
PORT_OFFSET=$(( (PORT_SEED % 60) * 50 ))

export RUN_NAME=${RUN_NAME:-063_T40_resume_370_to_1250_$(date +%Y%m%d_%H%M%S)}
export RAY_ADDRESS_OVERRIDE=local
export RAY_SESSION_TMPDIR_OVERRIDE=/tmp/r063_T40_${PORT_SEED}
...
export TEAM_DISTILL_ALPHA_INIT=0.05      # initial KL weight (mid; per snapshot-055 §2.5)
export TEAM_DISTILL_ALPHA_FINAL=0.0      # decay to 0
export TEAM_DISTILL_DECAY_UPDATES=8000   # half of typical training updates
export TEAM_DISTILL_TEMPERATURE=4.0      # snapshot-063: softer teacher distribution
...
LOG=docs/experiments/artifacts/slurm-logs/063_T40_temp_distill_train_$(date +%Y%m%d_%H%M%S).log
