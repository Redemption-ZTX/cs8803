#!/bin/bash
# 103B: Stone DIR-A Wave 3 — DEFENDER sub-task specialist (POSITIONING phase).
# Scenario: ball on own half + teammate has ball; controlled player should
# maintain support spacing (4-6m behind teammate), stay between teammate and own goal.
# Reward: opp_progress_penalty 0.05, defensive_survival_bonus, large possession_dist
# (encourages staying spread out, not crowding teammate).
set -euo pipefail
cd /home/hice1/wsun377/Desktop/cs8803drl

LANE_TAG=103B_defender
...
PORT_SEED=${PORT_SEED:-113}
PORT_OFFSET=$(( (PORT_SEED % 60) * 50 ))

export RUN_NAME=103B_defender_$(date +%Y%m%d_%H%M%S)
export RAY_ADDRESS_OVERRIDE=local
export RAY_SESSION_TMPDIR_OVERRIDE=/tmp/r103B_${PORT_SEED}
...
# Sub-task scenario: POSITIONING phase
export SCENARIO_RESET=defender_subtask
export BASELINE_PROB=0.0

# DEFENDER reward shape: defensive containment, support spacing
export USE_REWARD_SHAPING=1
export SHAPING_FIELD_ROLE_BINDING=0
export SHAPING_TIME_PENALTY=0.0005          # mild urgency
export SHAPING_BALL_PROGRESS=0.005          # low — defender doesn't push ball
export SHAPING_GOAL_PROXIMITY_SCALE=0.0
export SHAPING_OPP_PROGRESS_PENALTY=0.05    # main signal — punish opp advance
export SHAPING_POSSESSION_BONUS=0.0         # don't encourage charging the ball
export SHAPING_POSSESSION_DIST=2.5          # large — encourage spacing not crowding
...
export SHAPING_DEFENSIVE_SURVIVAL_THRESHOLD=300       # survival 300 step gives bonus
export SHAPING_DEFENSIVE_SURVIVAL_BONUS=0.02          # 0.02 / step after 300
export SHAPING_FAST_LOSS_THRESHOLD_STEPS=200          # fast loss penalty
export SHAPING_FAST_LOSS_PENALTY_PER_STEP=0.001
export SHAPING_EVENT_SHOT_REWARD=0.0
export SHAPING_EVENT_TACKLE_REWARD=0.05               # secondary
export SHAPING_EVENT_CLEARANCE_REWARD=0.10            # main event reward
...
LOG=docs/experiments/artifacts/slurm-logs/103B_defender_train_$(date +%Y%m%d_%H%M%S).log
