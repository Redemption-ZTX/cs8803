...
PORT_SEED=${PORT_SEED:-171}
PORT_OFFSET=$(( (PORT_SEED % 60) * 50 ))

export RUN_NAME=070_poolB_divergent_distill_scratch_$(date +%Y%m%d_%H%M%S)
export RAY_ADDRESS_OVERRIDE=local
export RAY_SESSION_TMPDIR_OVERRIDE=/tmp/r070_${PORT_SEED}
...
export TEAM_DISTILL_TEMPERATURE=1.0      # standard, no sharpening
...
LOG=docs/experiments/artifacts/slurm-logs/070_poolB_train_$(date +%Y%m%d_%H%M%S).log
