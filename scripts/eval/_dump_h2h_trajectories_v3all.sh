#!/bin/bash
# v3all: dump trajectories for ALL 30 pairs (8-model + 2 self-play).
# 500 episodes per pair, full per-step npz (raw obs + ball_xy via patched dumper).
# Used for Direction 1.b retrain on bigger dataset.
# Usage: bash _dump_h2h_trajectories_v3all.sh <NODE_INDEX 1..7>
set -euo pipefail
...
# 30 pairs: NAME T0_KIND T0_CKPT T1_KIND T1_CKPT (ensemble has CKPT="NONE")
declare -a PAIRS=(
...
TOTAL=${#PAIRS[@]}
echo "[node=$NODE_IDX] dispatching from $TOTAL pairs (mod 7)"

for i in "${!PAIRS[@]}"; do
  if (( (i % 7) + 1 != NODE_IDX )); then continue; fi
  ...
  LOG=docs/experiments/artifacts/official-evals/failure-capture-logs/dump_v3all_${NAME}.log
  mkdir -p "$SAVE_DIR"
  ...
  ENV_ARGS+=("PYTHONPATH=$PWD${PYTHONPATH:+:$PYTHONPATH}")

  echo "[node=$NODE_IDX] launching dump $NAME (port=$PORT) m0=$M0_MOD m1=$M1_MOD"
  env "${ENV_ARGS[@]}" \
